...
function see2gather(initData){
    ...
    var intId = setInterval(function(){
        if('jQuery' in window){
            clearInterval(intId);
            initsee2gather(initData);
        }else{
            console.log(new Date().getTime()+'::jquery doesnt exist yet...');
        }
    },delay);
...
function initsee2gather(initData) {
    ...
    if(initData){
        if(initData.shareAllElements)
            allControls = true;
        if(initData.loadImages)
            loadImages = true;
    }
    ...
    var shareLink = httplink+"/client.html#";
    var shareSupportLink = httplink+"/support.html#";
    ...
    $(document.head).append('<link rel="stylesheet" href="'+httplink+'2gather.css">');
    ...
    var getDocumentSnapshot = function() {
        ...
        return { role: 'server', uid:uid, type: 'replicate', data: doc, scrollTop: $(document).scrollTop(), scrollLeft: $(document).scrollLeft() };
    }
    ...
    var handleSessionStart = function() {
        ...
        connection = new WebSocket(wsprotolink);
        ...
        connection.onmessage = function(message) {
            ...
            if (json.type === 'client-update') {
                ...
                setTimeout(function() { $("#" + json.data.eleName).removeClass("focus-field"); }, 1000);
    ...
    var sendData = function(evtFor, eleName, eleVal, eleType, evtType, eleLabel, allowInput, type) {
        var evtData = { role: 'server', uid:uid, evtFor:evtFor, eleName:eleName, eleVal:eleVal, 
                            eleType:eleType, evtType:evtType, eleLabel:eleLabel, "allow-data-input": !!allowInput, type:type };
        if (sessionStatus === "in-progress"){
    ...
    handleImages(loadImages);

};


function handleImages(loadImages) {
    if(!loadImages)
        return;
        
    for (i = 0; i < document.getElementsByTagName('img').length; i++) {
        var fullFileName = document.getElementsByTagName('img')[i].src;
        if(loadedFiles[fullFileName] !== null)
            continue;

        
        var fileName = $(document.getElementsByTagName('img')[i]).attr('src');
        toDataURL(fullFileName, function (dataUrl) {
            loadedFiles[fullFileName] = true;
            var data = {
                dataUrl: dataUrl,
                src: fileName
            };
            $.post('/d/i', JSON.stringify(data), function (d) {
                console.log(d);
            });
        });
    }
